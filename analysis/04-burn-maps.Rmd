...
data <- readRDS(here("data/generated/WFC_map_data.rds"))
names(data) <- gsub("\\.", "", names(data))
library(dplyr)
library(rstan)
library(readr)
#data <- dplyr::rename(data, slope = Slope, prenbr = preNBR) # slope and prenbr already in lower clase
...
out <- plyr::mlply(.data = vars, function(x, y) {
  ...
  m <- fit_model(d, model = sm, predictors = "xscaled", iter = 100L, # changed from 1000L -> 100L
    chains = 1L) # changed from 4 -> 1L
...
xx <- purrr::map_df(out, function(x) {
  ...
  re_id <- x$d[x$d$group == "wfc", "group_id", drop = TRUE][[1]]
...
readr::write_rds(xx, here::here("data/generated/WFC_predictions.rds"))
```
...
out_q2 <- plyr::mlply(.data = vars, function(x, y) {
  ...
  m <- fit_model(d,
    ...
    iter = 100L, chains = 1L, adapt_delta = 0.98, seed = 42 #changed to 100L and 1L
  )
  list(m = m, d = d, x = x, y = y)
})
```
...
readr::write_rds(xx, here("data/generated/WFC_predictions_Q2.rds"))
```
...
```{r}
q2 <- readr::read_rds(here("data/generated/WFC_predictions_Q2.rds"))
q1 <- readr::read_rds(here("data/generated/WFC_predictions.rds"))
